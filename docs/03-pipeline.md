...
| リサイズ | **長辺 1024px**。作業グリッドと一致させる（v2）。小さい写真は拡大する（v2.6.2、§3.2.2） |
...
### 3.2.2 小さい写真は拡大してグリッドを埋める（v2.6.2、実写で判明）

> **実装で判明した設計の誤り（2026-09-08）。**
> v2.5 までは「拡大しても情報は増えず、ガウシアン数だけ無駄に増える」として
> 等倍で中央に置いていた。これが誤りだった。訂正する。

**どのモデルも作業グリッド全体を自分の入力寸法へ縮めて受け取る。** MODNet は
1024² を 512² に、u2netp は 320² に、DA3 は 518² に縮める。だからグリッドを
埋めないと、埋まっていない割合のぶんだけ被写体がモデルの中でさらに縮む。

268×512 の写真（実写）では:

| | 内容の大きさ | MODNet が見る人物 | u2netp が見る人物 |
|---|---|---|---|
| 等倍（v2.5） | 268×512 | 134×256 | 84×160 |
| 拡大（v2.6.2） | 536×1024 | 268×512 | 168×320 |

等倍だと **MODNet が胴を丸ごと落とした**（docs/09 §V14）。マットの面積が
MODNet 39,257px 対 u2netp 114,982px と 3 倍食い違い、門（§3.3）は
MODNet 側の穴をそのまま受け継ぐ。拡大すると 231,031 対 253,890 と一致する。

顔の landmark モデルの自己申告スコアも −6.1 → 19.1 に上がった。等倍では
顔が 55px しかなく、当たっていなかった。

**ガウシアン数は拡大率ではなくグリッドの広さで頭打ちになる**（被写体が
グリッドを超えることはない）ので、最悪値は大きい写真と同じである。密度を
落としたいときは⑦の統合率で決める。そちらが設計の意図である。
...
`scale = 1.3 · z / focalPx`、傾斜面では `1/max(|n·v|, 0.3)` 倍に伸ばす。リム処理（境界 2px の α 減衰・スケール 1.5 倍・法線の視線方向ブレンド）も同じ。

> **係数は v2.6.7 で 1.4 → 1.3。** 参照実装（SHARP）の「半径 ÷ 点間隔」は 0.70 で、私たちは 1.4 で 0.80、1.3 で 0.75 になる。1.2（0.69）まで落とすと、胴を 0.12 単位に切った至近で穴が 0.11% → 0.46% に増えたので 1.3 で止めた。半径を縮めると質感が戻る（至近の描画の勾配 4.61 → 4.72）。docs/11 §11.6 S5。
...
### 3.6.4 ⑥⑦ だけ写真の解像度で回す（v2.6.7）

**深度は 1024² のままでよい。** 深度モデルの入力が 518² なので、作業グリッドを
上げても情報は増えない。増えるのは**色**である。1116×2000 の写真を 1024² に
載せると中身は 571×1024 で、**線密度で 1.95 倍を捨てている**。§3.2 は
「色プレーンは写真そのものであり、ここが品質の天井になる」と書いているのに、
その天井を自分で下げていた。

そこで、⑥（スプラット生成）と⑦（適応サンプリング）と法線推定だけを
**作業グリッドの 1.5 倍**（1024² なら 1536²）で回す。

- 色は `prepareImageScaled` で写真からもう一度サンプルする。`letterbox` を
  取り直さず**同じ枠を 1.5 倍する**ので、深度・マットとの位置ずれが原理的に
  出ない。
- 深度とマットは双線形で 1.5 倍に伸ばす。情報は増えないが、色に合わせる。
- 焦点距離も 1.5 倍する。

**倍率はプリセットで変える（v2.6.9）。** 標準は 1.5 倍、**高品質は 2.0 倍**。
2.0 倍で「元写真の画素 1 個につきスプラット 1 枚」に届く（1116×2000 の写真なら
中身が 1142×2048 で元写真の 1.02 倍）。ここが上限で、これ以上は色を引き伸ばす
だけである。

**実測**（1116×2000 の立ち姿、reduction 0）:

| 倍率 | 中身 | 枚数 | `.splat` | 至近の勾配 | 至近の穴 | 法線＋⑦＋⑥ |
|---|---|---|---|---|---|---|
| 1.0（v2.6.6 まで） | 元写真の 0.51 倍 | 247k | 7.9 MB | 4.27 | — | 1.4 s |
| **1.5（標準）** | 同 0.77 倍 | 524k | 16.8 MB | 4.92 | 0.42% | 4.5 s |
| 1.75 | 同 0.90 倍 | 713k | 22.8 MB | 5.41 | 0.15% | — |
| **2.0（高品質）** | 同 **1.02 倍** | **930k** | **29.8 MB** | **6.02** | **0.01%** | **9.1 s** |
| 参照実装（SHARP） | — | 473k | — | 10.97 | 0.01% | — |

「至近の穴」は胴を 0.12 単位に切って描いたときに、スプラットの隙間から向こうが
見える割合。**2.0 倍で 0.01% になり、参照実装と並ぶ。**

**高品質だけにする理由。** ⑥⑦ の時間が 1.5 倍のときの 2.0 倍になり、10 秒の
予算（§3.9）をほぼ使い切る。`.splat` も 29.8 MB になる。標準プリセットは
1.5 倍のままにして、代償を承知の人だけが高品質を選ぶ形にする。

倍率は作業グリッドへの**比**で持つ。軽量プリセット（512²）でも同じ割合で効き、
...
| ④⑤⑥ | 前面・背面シェル・スカート幾何（1536²、§3.6.4） | 0.54 s | 4.83 | 作業グリッドで回す |
| ⑦ | 適応サンプリング・ドキュメント確定（1536²） | 0.25 s | **5.08** | 作業グリッドで回す |
| | **▶ プレビュー表示・操作可能** | | **5.08 s** | |
| ⑧ | 遮蔽部インペイント（MI-GAN 512²） | 0.90 s | 5.98 | 失敗時は引き伸ばし色のまま |
| ⑨ | 継ぎ目補正 60 反復 @256² | 0.60 s | 6.58 | 反復数を 30 に |
| ⑩ | 量子化・画像パッキング（1024²） | 0.80 s | **7.38** | **書き出し時まで遅延** |
| | **▶ 全処理完了** | | **7.38 s** | **上限 10.0 s** |

余裕 2.62 秒。**v2.6.7 で ④⑤⑥⑦ を 1.5 倍のグリッドで回すようにしたぶん、0.29 秒
狭くなっている**（この機械の実測で サンプリング ×1.65、組み立て ×1.55 だったのを
表の単位に掛けた）。シフト推定（約60ms）が閉形式化で不要になったぶん、v2 より少し広い。v1（余裕 2.4 秒）より広がっているのは、微調整の 2.4 秒がインペイント 0.9 秒と継ぎ目補正 0.6 秒に置き換わったため。
