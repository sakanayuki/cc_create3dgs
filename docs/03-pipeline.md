...
**実測**（1116×2000 の立ち姿、高品質プリセット）:

| | 枚数 | `.splat` | 法線 | サンプリング | 組み立て |
|---|---|---|---|---|---|
| 1024²（従来） | 247k | 7.9 MB | — | 0.63 s | 0.73 s |
| **1536²** | **536k** | **17.2 MB** | 0.46 s | 1.11 s | 1.09 s |
| 2000²（写真の実寸） | 894k | 28.6 MB | 0.74 s | 1.94 s | 3.00 s |

至近の描画の勾配は 4.27 → 4.74（1536²）→ 5.70（2000²）。**2000² は採らない。**
+4.3 秒で 10 秒の予算（§3.9）に入らず、`.splat` も 29 MB になる。
