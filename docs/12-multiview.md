...
### 12.15.3 実素材での測定（PoC-3A、**まだ合格していない**）
...
`scripts/align_probe.ts`（それを読んで位置合わせを回し、M1 を出す）。
    ...
    python3 scripts/multiview_probe.py --front tests/test29.jpeg \
        --right tests/test29_left.jpeg --left tests/test29_right.jpeg --out tests/multiview-probe
...
| A2 | **腕を外すと角度が大きく変わる。** 被写体まるごとで 61°/−46°、胴と頭だけで 52°/−124° | D33（腕は正面だけ）と §12.7（合わせは胴と頭だけ）の前提は、効きの向きとしては正しい |
