...
| 道具 | `scripts/render_probe.ts` / `render_ply.py` | **済** 合成結果を**絵にして見る**（§12.16.7） |
...
出る（尺度を揃えて2枚ずらすと基準が 0.874 で最下位）。そのときは**1枚も外さずに
降りる**。

**ここは最初、書いたとおりに実装できていなかった**（PR #8 の Codex の指摘 P1）。
コードは基準を飛ばして「非基準のうち最下位」を無条件に外していた。すると残るのは
「基準 ＋ もう1枚のずれた view」で、しかも2枚になると合格ラインが 0.85 に緩むので、
**そのまま通って壊れた立体が出る**。ゲートが止めるはずのものを、ゲートの手前で
作っていた。この節に降伏すると書いておきながら、コードに書いていなかった。

合成データで再現する（幅 2.5 倍・奥行き 0.4 倍の体を横2枚に置く）。
...
最下位が基準で、かつ3枚の合格ライン 0.90 を割っている。直す前は右向きを外して
2枚で解き直し、0.85 を通して合成していた。直したあとは1枚も外さず、基準1枚に
降りる。`tests/unit/multiGate.test.ts` に固定した（直す前は落ちることも確かめた）。
...
### 12.16.7 実機で「1枚より悪い」と言われた。位置合わせが効いていなかった

重複除去を出したあと、実機で3枚を通した依頼者からこう報告を受けた。

> 三枚の画像が重なり合っただけで、1枚のみよりも正面の見た目が悪く、
> 人物に見える角度がありません

**そのとおりだった。しかも、こちらが測っていた数字はすべて「良い」と言っていた。**
M1 0.969、二重率 2.5%、点数もバジェット内。それでも絵は壊れていた。

#### まず、絵を見る道具を作った

ここまで**一度も絵を見ずに**数字だけで進めていた。それが根本の失敗である。
`scripts/render_probe.ts`（変種ごとに PLY を書く）と `scripts/render_ply.py`
（正射影で PNG にする）を足した。本番の書き出しと同じ `splatsToPly` を通すので、
見ているものと利用者が保存するものがずれない。

これで**実機の破綻がオフラインで再現した**。以後、直すたびに絵で確かめられる。

#### 外れた仮説を2つ記録する

1. **スカートのせい** — 横向き view のスカートは、法線がその写真の画像平面内に
   あるので、±90° 回すと正面カメラを向く。実際、合成後に正面を向いたスカートが
   10,005 枚あった（正面 view 自身のスカート 5,810 枚は設計どおり真横で見えない）。
   もっともらしかったが、**スカートを切っても絵はほとんど変わらなかった**。
2. **角度の過小回転のせい** — 解は 72.7°/−77.9° で ±90° に届いていない。手で
   ±90° に置いて描いてみたが、**わずかに良くなるだけ**だった。

#### 本当の原因: 回転の中心

view ごとに色を塗り分けて正面から描いたら、一目で分かった。**横向きの2枚が、
体の左右ではなく「真ん中」に縦の板として置かれていた。**

| view | 解いた yaw | 置かれた x の範囲 | 中心 x |
|---|---|---|---|
| 正面 | 0.0° | [−0.172, +0.169] | 0.000 |
| 右向き | 72.7° | [−0.130, +0.018] | **−0.077** |
| 左向き | −77.9° | [−0.038, +0.094] | **+0.059** |

体の幅は ±0.17 なのに、横向きの面は中央（|中心| ≤ 0.08）にある。しかも
z が 0.641 と、正面が観測した面（0.669）より**手前**に出ている。正面カメラが
見たはずの場所に別の面があるので、顔の上に淡い破片が乗る。

原因は `frames`（回転の中心）の取り方だった。**各 view が見ているのは被写体の
「手前の面」だけで、その重心は回転軸ではない。** 軸はそこから体の厚みの半分ぶん
奥にある。重心どうしを重ねると、横向きの面が厚みの半分ぶん内側に寄り、90° 回れば
それがそのまま横位置のずれになる。ずれの大きさ（約 0.11）は観測した潰れ量と一致する。

#### M1 が壊れ方を隠していた

**収まりだけで解くと、全部を体の内側へ潰し込むのがいちばん安い。** 横向きの面を
体の真ん中に置いても正面のシルエットの内側には収まるので、収まりの項は文句を
言わない。M1 = 0.969 という高い値は、正しく合ったからではなく**潰れているから**
出ていた。

**M1 が高いことは必要条件にすぎない。** 置き場所が正しいことは何も言っていない。
§12.16.5 で「両側を測れ」と書いたが、測る量そのものが見え方を代表していなかった。

#### 試したこと（採否）

| 直し方 | 実素材の yaw | 実素材の絵 | 合成データの検査 |
|---|---|---|---|
| いまの実装（重心を軸にする） | 72.7° / −77.9° | **壊れている** | 通る |
| 自由空間の項を目的関数へ（重み 0.5） | 112.4° / −100.6° | 悪化 | — |
| 自由空間の項（重み 1.5） | 117.4° / −110.9° | 悪化 | — |
| **回転の中心を「手前の面 + 厚み/2」へ** | **93.6° / −87.0°** | **人物に見える** | **3件落ちる** |
| 上 ＋ 合成後に自由空間の点を落とす | 同上 | **いちばん良い** | 同上 |

- **自由空間を目的関数に入れるのは誤り**だった。手前へ出た量を罰すると「面を奥へ
  逃がす」方向に効き、余計に回してでも逃がそうとする。重みを上げるほど角度が
  ±90° を通り越した。`W_FREESPACE = 0` にして記録だけ残す。
- **回転の中心の修正は、実素材では決定的に効く。** yaw が ±90° を挟むようになり、
  横向きの面が体の端へ寄り、顔が出た。
- **しかし合成データの検査が3件落ちる**（3枚で 85°、2枚で 106.25°）。実素材が
  良くなって合成が悪くなる。**片方だけを見て採用はできない。**
- 合成後のカリング（カメラが見通した空間の点を落とす）は、**軸を直したときだけ**
  効く。軸が元のままだと、かえって髪や体が崩れる。

#### 今回出荷したもの／出荷しなかったもの

**挙動は変えていない。** 軸の修正は合成データの検査を落とすので、通るまで本番の
経路には入れない。検査を緩めて通すことはしない（そのために置いた検査である）。

出荷したもの。

- 絵を見る道具（`render_probe.ts` / `render_ply.py`）
- `mergeBuilds` の `occluder`（カメラが見通した空間の点を落とす）。**本番からは
  まだ渡していない**。道具からは使う
- この記録
- 画面に「実験中。正面は1枚モードのほうがきれい」と明記した。原因と見通しも書いた

#### 次にやること

合成データと実素材が食い違う理由を詰める。合成の体（`syntheticBody.ts`）は
回転軸を `axisZ: 3.2` と明示して描いているので、**真の軸が分かっている**。
新しい軸の見積もりがその値をどれだけ外しているかを直接測れば、見積もりの式が
悪いのか、目的関数が別の谷へ落ちているのかを分けられる。そこを押さえてから
本番へ入れる。
