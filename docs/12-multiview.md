...
**呼び方をここで決める。** 「右向き」とは**写真の中で被写体が画面の右を向いている**もの、
「左向き」は画面の左を向いているものとする。撮った人が見たままの呼び方で、迷いようがない。
ヨー θ の符号は「画面の右を向いている = θ > 0」と一度決め、コードのコメントに残す。
...
| O3 | 2枚の横向きは**逆向き**。腰のリボンが片方にだけ見える | 正面・右向き・左向きで、水平約 270° を覆える |
...
1. **横向きでの検証。** MODNet は正面寄りのポートレートを想定した学習である。
   ...
   自動で切り替える（+1.1 s／view）。
2. **view 間の整合を、身長で見る。** 各 view のマットから測った被写体の高さ（画素→ metric）が
...
| 右向き + 左向き（正面なし） | 約 180°（正面と背面が抜ける） | θ の差が約 180° で、**共視する面がほとんど無い**。頼れるのはシルエットと身長だけ | 動かすが、M1 が閾値を割ったら片方だけの結果に落とす。UI で「正面を足すと良くなる」と伝える |
...
| 新規 | `tests/multiview/{front,right,left}.jpg` | **実素材3枚**（§12.17） |
...
| 問い | 判断 | 反映先 |
|---|---|---|
| 素材の3枚をリポジトリに置くか | **置いてよい** | `tests/multiview/`（§12.17.1） |
...
### 12.17.1 素材の置き場所（**まだ置かれていない**）

置き場所と名前を決めて `tests/multiview/README.md` に書いた。**画像そのものはまだ無い。**
会話に貼られた3枚は手元のファイルとして受け取れていないので、次の3つの名前で
リポジトリに追加していただきたい。

| ファイル | 中身 |
|---|---|
| `tests/multiview/front.jpg` | 正面 |
| `tests/multiview/right.jpg` | 画面の右を向いた横向き |
| `tests/multiview/left.jpg` | 画面の左を向いた横向き |

**PoC-3A はこれが揃うまで始められない。** 合成データによる検算（§12.15）は素材が無くても
書けるので、着手するならそちらが先になる。

### 12.17.2 残っている問い

1. **R20 の再試行を、どこまで自動でやるか。** 符号の反転1回までは自動でよいと考えているが、
   「利用者が指定した枠を勝手に読み替える」ことでもある。黙って直すか、断ってから直すか。
2. **WASM 経路で複数枚モードを出すか**（§12.12）。PoC-3D の実測待ち。第一候補は WebGPU 限定。
3. **2枚モードのプリセット。** 3枚と同じ扱いでよいか、それとも2枚では高品質プリセットを既定にして
