...
- **深度バケットの計数ソート**（1パス、8192 バケット）を WGSL で実装。被写体が
  正規化されていて深度の幅が既知なので、基数ソートの 4 パスは要らない。
- カリング後の約 240,000 要素で約 2.5 ms

### バケットの範囲は「そのフレームのカメラからの距離」で張る（v2.6.7）

```
R = SCENE_RADIUS = 0.9        # 一辺 1 の立方体を包む球（半対角 0.866）に余裕
バケット = [d − R, d + R] を 8192 等分     # d はカメラから注視点までの距離
```

**生成時のカメラ（距離 1.0）の深度レンジを使ってはいけない。** ビューアの距離は
0.4〜1.667 まで動くので、寄ると実際の距離がその範囲を外れ、`clamp` で全部が
1 バケットに潰れる。そうなると順序が消え、奥の面が手前に出て**透けて見える**。
既定の視点がちょうど距離 1.0 なので、最初の一枚だけ正しく見えて、触った瞬間に
壊れる。実測と検査は docs/09 §V22。

バケット幅は被写体 1m で 0.22mm。サーフェル 1 個より細かいので、バケット内の
順序が不定でも α ブレンドの見た目に影響しない。
