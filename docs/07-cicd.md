...
|---|---|
| 生成が例外なく完了するか | 必須 |
| ガウシアン数が想定範囲内か | 標準プリセットで 70,000 〜 130,000 |
| `.pgs` のサイズ | < 500 KB |
| 生成画像と入力画像の **SSIM**（入力視点から描画） | > 0.90 |
| ±40° 回転時に穴（α=0 の画素）が被写体領域の何%か | < 2% |
...
// budget.json
{
  ...
  "models": {
    "初回必須 (深度 + 人物マット)": { "warn": 25600000, "fail": 30000000 },
    "物体モード追加分":             { "warn": 44100000, "fail": 50000000 },
    "合計":                        { "warn": 70000000, "fail": 80000000 }
  },
  "output": {
    "pgs.bytes":      { "warn": 350000, "fail": 500000 },
    "gaussianCount":  { "warn": [70000, 110000], "fail": [50000, 130000] }
  },
  "quality": {
    "ssim.inputView": { "warn": 0.92, "fail": 0.90 },
    "holeRatio.40deg":{ "warn": 0.01, "fail": 0.02 }
  },
  "pages": {
...
| 帯域 | 100 GB / 月（ソフト） | モデル 70 MB × 訪問者。**月1,400人で上限**。→ §7.7 |
...
**最も現実的なリスク。** モデル 70 MB を毎回配信すると、月100 GB は約1,400訪問で尽きる。

| 対策 | 効果 |
|---|---|
| `Cache API` にモデルを保存し、リピート訪問では再取得しない | 実質的に初回のみ。ただし GitHub Pages のキャッシュヘッダは制御できないため、`Cache API` に明示的に入れる実装が必須 |
| 物体モードのモデル（44 MB）を**遅延ロード**する | 人物モードのみの訪問者は 25.6 MB で済む。初回訪問の帯域を約 1/3 に |
| ファイル名にハッシュを含め、`Cache API` のキーにする | モデル更新時のみ再取得 |
| バジェットでモデル合計を 80 MB に制限 | 際限なく増えるのを防ぐ |

これらを適用しても、**月あたり約 3,900 訪問で上限に達する**（初回訪問の平均を 25.6 MB と仮定）。
