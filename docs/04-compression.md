...
### 4.6.3 q4f16 には WebGPU の `shader-f16` が要る（v2.2、実測で判明）

**この拡張が無い WebGPU 実装では、q4f16 のモデルは黙って壊れる。** 例外は投げられず、推論は「成功」して返る。

本物のモデルを手元に落として通しで動かしたところ、次のことが起きた。

| | `shader-f16` 無しで q4f16 を使う | uint8 に切り替えた後 |
|---|---|---|
| DA3 の `intrinsics` の `fx` | **0**（CPU 実行では 887.5） | **740.9** |
| 焦点距離 | 画角 55° の仮定に落ちる | モデルの推定値を採用 |
| スカート（深度の段差由来） | **0 枚** | 46,212 枚 |
| コンソール | `'f16' type used without 'f16' extension enabled` が多数 | なし |

スカートが 0 枚だったのは、深度マップに段差が1つも無かったからで、つまり**深度そのものが出来ていなかった**。それでも生成は完走し、それらしい立体が表示されていた。**画面を見ているだけでは気付けない壊れ方である。**

対処（`src/runtime/modelCatalog.ts` の `manifestBackendKey`）: アダプタが `shader-f16` を持たない WebGPU では、q4f16 ではなく uint8 側（WASM 用に置いてあるもの）を落とす。実行プロバイダは WebGPU のままでよく、変えるのは**どのファイルを落とすか**だけ。D22 の分岐がそのまま使える。

どちらを使ったかは画面に出す（「shader-f16 が無いため uint8 モデルを使用」）。黙って遅い・重いほうを使うと、原因の分からない性能差になる。
