...
#### 座標系は「カメラを原点にした実寸メートル」（v2.6.9）

⑥ は描画のために被写体を**一辺 1 の立方体**へ正規化するが、**書き出しはそれを
戻す**。参照実装（SHARP）の出力もそうなっており、他の資産と並べる・寸法を測る、
といった使い方はこれでないとできない。

```
書き出す座標 = (正規化座標 ÷ normalization.scale + normalization.center) × metricFix
半径         =  正規化半径   ÷ normalization.scale                        × metricFix
```

`metricFix` は深度モデルの絶対値の偏りを直す倍率で、人物モードでは
**立ち姿の身長 1.65m を仮定**して求める（`statureFix`）。深度モデル（DA3）は
実測で「立っている人の身長 0.43〜0.46m」と返してくるので、これが無いと
かえって小さくなる。**推定であって実測ではない**こと、バストアップの
切り抜きでは外れることを含め、docs/09 §V25 に測定と限界を書いた。

位置も半径も同じ倍率で動く相似変換なので、**形も見た目も変わらない**。
変わるのは「1 単位が何メートルか」だけである。
