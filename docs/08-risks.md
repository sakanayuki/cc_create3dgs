...
| ID | リスク | 影響 | 確率 | 対策 |
|---|---|---|---|---|
| ~~R1~~ | ~~ORT の WebGPU EP が動かない~~ | — | — | **PoC-1 で顕在化**。WebGPU 自体が使えない端末が存在したため、D19 でフォールバックを設計に組み込んで解消 |
...
| ~~R10~~ | ~~DA3-Small が動かない／内部パラメータが無い~~ | — | — | **PoC-1 で解消。** 動作し、`intrinsics` も `confidence` も出力する |
| **R11** (v2) | MI-GAN の補完品質が物体の模様で不足する | 中 | 中 | §8.9 |
| **R12** (v2) | 1024²・42万ガウシアンで中位端末が 60 fps を割る | 中 | **高** | §8.10。WebGPU 端末が手に入るまで未検証のまま |
| **R13** (v3) | WASM 経路が SLO に収まらない | **大** | **高** | PoC-1 実測で単スレッド 30〜63 秒。D20（マルチスレッド、約3倍）が前提。効果は PoC-2 で実測する |
| **R14** (v3) | WebGL2 レンダラの実装コストと品質 | 中 | 中 | WGSL 版と同じ見え方にする必要がある。深度バケット計数ソートは WebGL2 では transform feedback か CPU ソートになる |
